...
The global functions defined below are also used in Appendices A-H. For example, the `nll()` function is our penalized negative log-likelihood which we introduced in Appendix C. The profile likelihood approach is similar to that used in `r citep("10.1002/eap.1421")`.
...
bootstrapping <- function(start.par)
{
  ...
  for(i in 1:3)
  {
    ...
    pred2 <- biphasicPlot(h=app2[1],t1=-app2[2]/app2[1],g=app2[3],Tmat=app2[5])
...
write.bibtex(file="Appendix_G_references.bib")
```
