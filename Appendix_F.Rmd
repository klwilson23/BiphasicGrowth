...
In this case, we will use the profile likelihood approach (`r citep("10.1002/eap.1421")`) See Appendix D for details.
...
We then show a percent bias plot that suggests that parameters for this one randomized dataset were estimated well, apart from $t_1$ (which was estimated with noise). We could repeat this trial many times to get a general idea of how well the model recovers true life-history parameters of interest (we do this in Appendix G).
