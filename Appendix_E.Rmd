...
In this case, we set the 'true' $g$ value at 70% of the intrinsic maximum. We also adjust sample sizes-at-age in an attempt to account for factors such as gear selectivity and natural mortality; however, we could simulate this using a multinomial process as well (see Appendix D).
...
We then show a percent bias plot. Our results suggest that parameters for this one randomized dataset were estimated well, apart from the parameter $t_1$, which was estimated with noise. We could repeat this trial many times to get a general idea of how well the model recovers true life history parameters of interest (we do this in Appendix I).
...
write.bibtex(file="Appendix_E_references.bib")
```
